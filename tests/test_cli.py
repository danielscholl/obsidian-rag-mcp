...
class TestSearchValidation:
    ...
    def test_search_top_k_zero_error(self):
        ...
        runner = CliRunner()
        with tempfile.TemporaryDirectory() as tmpdir:
            result = runner.invoke(cli, ["search", "test", "--vault", tmpdir, "--top-k", "0"])
            assert result.exit_code != 0
    ...
    def test_search_top_k_negative_error(self):
        ...
        runner = CliRunner()
        with tempfile.TemporaryDirectory() as tmpdir:
            result = runner.invoke(cli, ["search", "test", "--vault", tmpdir, "--top-k", "-5"])
            assert result.exit_code != 0
            assert "must be at least 1" in result.output
    ...
    def test_search_top_k_too_large_error(self):
        ...
        runner = CliRunner()
        with tempfile.TemporaryDirectory() as tmpdir:
            result = runner.invoke(cli, ["search", "test", "--vault", tmpdir, "--top-k", "51"])
            assert result.exit_code != 0
    ...
    def test_search_top_k_valid_boundary(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            result_min = runner.invoke(cli, ["search", "test", "--vault", tmpdir, "--top-k", "1"])
            result_max = runner.invoke(cli, ["search", "test", "--vault", tmpdir, "--top-k", "50"])
